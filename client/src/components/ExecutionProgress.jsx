    ...
    <div className="border border-gray-200 rounded-lg bg-white p-4 space-y-3">
      ...
      {autoSaveStatus && (
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          {recoveryUrl && (
            <a href={recoveryUrl} download={`BRAT_Recovery_${completed}rows.json`} className="text-[#39b6e6] hover:underline ml-1">
      ...
      {stallWarning && stallWarning.type === 'soft' && !isAutoPaused && (
        <div className="bg-amber-50 border border-amber-200 rounded px-3 py-2 text-xs text-amber-700">
          <p className="font-semibold">Stall detected: no results in {Math.round((timeSinceLastResult || 0) / 1000)}s.</p>
      ...
      {circuitBreak && isAutoPaused && (
        <div className="bg-red-50 border border-red-200 rounded px-3 py-2 text-xs text-red-700 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={onResumeSlow} className="bg-amber-500 hover:bg-amber-600 text-white px-3 py-1 rounded text-xs font-medium">Resume slowly (1 worker)</button>
            {onResume && <button onClick={onResume} className="bg-[#39b6e6] hover:bg-[#2d9bc4] text-white px-3 py-1 rounded text-xs font-medium">Resume normal</button>}
            <button onClick={onCancel} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-medium">Cancel batch</button>
          </div>
        </div>
      )}
      ...
      {/* Auto-paused (high error rate or hard stall, not circuit breaker) */}
      {isAutoPaused && !circuitBreak && (
        <div className="bg-red-50 border border-red-200 rounded px-3 py-2 text-xs text-red-700 space-y-2">
          <p className="font-semibold">
            {stallWarning?.type === 'hard'
              ? `Auto-paused: stall detected (no results for ${Math.round((timeSinceLastResult || 0) / 1000)}s).`
          ...
          <div className="flex gap-2">
            <button onClick={onResumeSlow} className="bg-amber-500 hover:bg-amber-600 text-white px-3 py-1 rounded text-xs font-medium">Resume at lower speed</button>
            {onResume && <button onClick={onResume} className="bg-[#39b6e6] hover:bg-[#2d9bc4] text-white px-3 py-1 rounded text-xs font-medium">Resume normal</button>}
            <button onClick={onCancel} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-medium">Cancel</button>
            {recoveryUrl && (
              <a href={recoveryUrl} download={`BRAT_Recovery_${completed}rows.json`} className="bg-[#002144] hover:bg-[#003366] text-white px-3 py-1 rounded text-xs font-medium inline-flex items-center">
                Download recovery
              </a>
            )}
          </div>
